...
- 迁移 `012_behavior.sql` 同时创建 `interview_behavior` 表并给 `interview_sessions` 加 `camera_enabled` 列
...
- Backend: `internal/behavior`（service.go + handler.go + repo.go + 测试）；`interview` 的 Session 模型/repo INSERT/SELECT/scan 加 `camera_enabled`；迁移 `012_behavior.sql`
