...
for i in $(seq 1 30); do
  if docker exec interviewassistant-mysql-1 mysqladmin ping -uroot -p123456 >/dev/null 2>&1; then
    break
  fi
...
# ---------- 4. 首次启动自动执行数据库迁移 ----------
TABLES=$(docker exec interviewassistant-mysql-1 mysql -uroot -p123456 -N -e \
  "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='interview'" 2>/dev/null || echo "0")
if [ "$TABLES" = "0" ]; then
  echo ">> 首次启动，执行数据库迁移 ..."
  for f in backend/migrations/*.sql; do
    docker exec -i interviewassistant-mysql-1 mysql -uroot -p123456 interview < "$f"
  done
fi
...
GOTMPDIR="$ROOT/backend/.gotmp" GOCACHE="$ROOT/backend/.gotmp/cache" \
  go build -o backend/server_docker.exe backend/cmd/server
