...
func TestStartInjectsWeakDimensions(t *testing.T) {
	...
	session, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, llm.StandardPersona, llm.StandardDifficulty, llm.StandardCompanyStyle, nil)
	if err != nil {
...
func TestCreatePersistsPersona(t *testing.T) {
	...
	session, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, "strict_tech", llm.StandardDifficulty, llm.StandardCompanyStyle, nil)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestCreateDefaultsStandardPersona(t *testing.T) {
	...
	svc := interview.NewService(sqlDB, &fakeLLM{}, store)
	session, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, "", llm.StandardDifficulty, llm.StandardCompanyStyle, nil)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestCreateRejectsInvalidPersona(t *testing.T) {
	...
	svc := interview.NewService(sqlDB, &fakeLLM{}, store)
	_, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, "evil", llm.StandardDifficulty, llm.StandardCompanyStyle, nil)
...
func TestStartUsesPersonaInPrompt(t *testing.T) {
	...
	capLLM := &capturingLLM{}
	svc := interview.NewService(sqlDB, capLLM, store)
	session, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, "warm_hr", llm.StandardDifficulty, llm.StandardCompanyStyle, nil)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if _, _, err := svc.Start(ctx, userID, session.ID); err != nil {
...
func TestCreatePersistsPrecheckGaps(t *testing.T) {
	...
	svc := interview.NewService(sqlDB, &fakeLLM{}, store)
	session, err := svc.Create(ctx, userID, "Backend engineer JD", nil, interview.ModeMixed, interview.InputModeText, llm.StandardPersona, llm.StandardDifficulty, llm.StandardCompanyStyle, []string{"缺少K8s经验"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	got, _, _, err := svc.Get(ctx, userID, session.ID)
