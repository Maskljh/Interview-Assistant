...
5. **（可选）摄像头分析**：勾选后，**仅当作答方式为「语音」时**（创建时选了语音，且未切换为文字），在面试间本地采集摄像头画面做表情/行为分析（见下节），实时显示紧张度指示灯；切换为文字作答时自动停止。
6. **结束**：正常结束（WS `done`）或强制结束（HTTP）。
7. **报告**：查看四维评分、优缺点、建议；如勾选了摄像头分析，另有「行为信号（辅助参考）」卡片。

## 摄像头表情/行为信号分析（V14）

创建面试时勾选「开启摄像头分析（可选）」（默认关闭）后，**仅当作答方式为「语音」时**，面试期间在**浏览器本地**对摄像头画面做实时分析，作为报告中的**辅助反馈**——**不参与四维评分**，不引入数字人形象。若面试中切换为文字作答，分析自动停止。

### 工作原理（隐私优先）

1. **采集**：`getUserMedia` 获取摄像头画面（仅在用户勾选开启**且**作答方式为语音并授权后）。
